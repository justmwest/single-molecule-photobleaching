...
def gen_radius_bins(distribution, n_bins=100000, n_std_devs=5):
    """
    Takes a distribution and the number of bins (int)
    and returns a list of numbers within n_std_devs standard deviations
    of the mean of the distribution.
    """
    smallest = distribution.mean() - (distribution.std() * n_std_devs)
    largest = distribution.mean() + (distribution.std() * n_std_devs)
...
def calc_number_of_smalps(total_system_area, distribution, n_bins=25000):
    ...
    radius_list = gen_radius_bins(distribution, n_bins)
